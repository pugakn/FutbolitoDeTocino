...
void Ball::Move(VECTOR4D& acceleration)
{
	acceleration.z = 0;
	//if (!_coliding)
		_acceleration = (-1/4024.f) * acceleration;
	...
	_velocity.z = 0;*/
	FOR(i, _mesh.m_Vertices.size()) _mesh.m_Vertices[i] = _mesh.m_Vertices[i] + _velocity;
...
{	
	//vector<VECTOR4D> m_Transformers;
	glEnableClientState(GL_VERTEX_ARRAY);
	//FOR(i, _mesh.m_Vertices.size())
	//{
	//	m_Transformers[i] = traslationMTRX*_mesh.m_Vertices[i];
	//}
...
void Ball::Initialize(int32_t w, int32_t h)
{
	...
	_mesh.m_Original = _mesh.m_Vertices;
}
